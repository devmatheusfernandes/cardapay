    ...
    return (
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-200/60 bg-white/80 backdrop-blur-sm">
          <AnimatePresence mode="wait">
            {!isCollapsed && (
              ...
              >
                <Link href="/dashboard/menu" className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                  Cardapay
                </Link>
              </motion.div>
            )}
        ...
        <div className="p-3 border-t border-slate-200/60 bg-white/80 backdrop-blur-sm">
          ...
          >
            <LogOut className={`${isCollapsed ? 'w-6 h-6' : 'w-5 h-5'} transition-all`} />
            <AnimatePresence>
              {!isCollapsed && (
                <motion.span
                  initial={{ opacity: 0, x: -10 }}
            ...
            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 hidden group-hover:block w-56 bg-white rounded-2xl shadow-xl border border-slate-200/60 z-50 backdrop-blur-md">
              <div className="p-2">
                {navItems.filter(item => 
                  !['/dashboard/profile', '/dashboard/menu', '/dashboard/orders', '/dashboard/analytics'].includes(item.href)
                ).map((item) => {
                  const isActive = pathname.startsWith(item.href);
                  return (
                    <Link

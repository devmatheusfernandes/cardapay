...
export default function OrderListItem({ orderId }: OrderListItemProps) {
  ...
  return (
    <li key={orderId} onClick={()  => router.push(`/track/${orderId}`)} className="bg-white rounded-xl shadow-sm transition-all hover:shadow-md relative cursor-pointer">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Pedido <span className="font-mono">{orderId.substring(0, 12)}...</span>
            </p>
          ...
          <div className="text-right">
            <div className={`flex items-center justify-end gap-2 font-semibold ${statusColor}`}>
              <StatusIcon className="w-5 h-5" />
              <span>{order.status}</span>
            </div>
            <button onClick={handleCopyClick} className="mt-2 flex items-center gap-1 text-xs text-slate-500 hover:text-rose-600 transition-colors">

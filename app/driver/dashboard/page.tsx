...
export default function DriverDashboardPage() {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center min-h-screen"><LoaderCircle className="w-12 h-12 text-rose-600 animate-spin" /></div>;
  }
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <header className="bg-white shadow-md">
        <div className="max-w-4xl mx-auto p-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-rose-600">Painel do Entregador</h1>
          <button onClick={handleSignOut} className="flex items-center gap-2 text-sm text-slate-600 hover:text-rose-600"><LogOut className="w-5 h-5" /> Sair</button>
        </div>
      </header>
        ...
        {!profile?.restaurantId ? (
            <div className="text-center py-20 bg-white p-8 rounded-lg shadow-md">
                ...
                <div className="mt-6 flex items-center justify-center gap-2 p-4 bg-slate-100 rounded-md max-w-xs mx-auto">
                    <span className="text-3xl font-mono font-bold text-rose-600 tracking-widest">{profile?.code}</span>
...
const DeliveryCard = ({ order, onConfirmClick }: { order: Order; onConfirmClick: () => void }) => (
    <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="mt-4 pt-4 border-t border-slate-200">
        <p className="font-bold text-slate-700 flex items-center gap-2"><MapPin className="w-5 h-5 text-rose-500" /> Endereço:</p>

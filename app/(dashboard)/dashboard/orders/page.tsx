...
export default function OrdersPage() {
    ...
    const ordersByDay = useMemo(() => {
        ...
        filteredOrders.forEach(order => {
            ...
            const dayKey = date.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
            if (!daysMap.has(dayKey)) {
                daysMap.set(dayKey, []);
            }
            daysMap.get(dayKey)?.push(order);
        });
        return Array.from(daysMap.entries()).sort(([a], [b]) => new Date(b).getTime() - new Date(a).getTime());
    }, [filteredOrders]);
...
const DaySection = ({ day, orders, displayedStatuses, isExpanded, onToggleExpand, onUpdateStatus, onAssignDriverClick, onCancelClick }: { day: string; orders: Order[]; displayedStatuses: Order['status'][]; isExpanded: boolean; onToggleExpand: () => void; onUpdateStatus: (id: string, newStatus: Order['status']) => void; onAssignDriverClick: (order: Order) => void; onCancelClick: (order: Order) => void; }) => (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <button onClick={onToggleExpand} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                    <div className="p-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {displayedStatuses.map(status => {
                            ...
                            return (
                                <OrderColumn key={status} status={status} orders={statusOrders} onUpdateStatus={onUpdateStatus} onAssignDriverClick={onAssignDriverClick} onCancelClick={onCancelClick} />
                            );
                        })}
                    </div>
                </motion.div>
...
const FilterButton = ({ label, icon: Icon, isActive, onClick }: { label: string, icon?: React.ElementType, isActive: boolean, onClick: () => void }) => ( <button onClick={onClick} className={`flex items-center justify-center gap-2 px-4 py-1.5 rounded-md text-sm font-semibold transition ${isActive ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:bg-slate-100'}`}> {Icon && <Icon className="w-5 h-5" />} {label} </button> );

const OrderColumn = ({ status, orders, onUpdateStatus, onAssignDriverClick, onCancelClick }: { status: Order['status']; orders: Order[]; onUpdateStatus: (id: string, newStatus: Order['status']) => void; onAssignDriverClick: (order: Order) => void; onCancelClick: (order: Order) => void; }) => {
    ...
    return (
        <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
            <div className="flex items-center gap-3 mb-3">
                ...
                <span className={`ml-auto text-xs font-semibold ${config.color} ${config.bg} px-2 py-1 rounded-full`}>{orders.length}</span>
            </div>
            <div className="space-y-3">
                {orders.map(order => (
                    <OrderCard key={order.id} order={order} onUpdateStatus={onUpdateStatus} onAssignDriverClick={onAssignDriverClick} onCancelClick={onCancelClick} />
                ))}
            </div>
        </div>
    );
...
const OrderCard = ({ order, onUpdateStatus, onAssignDriverClick, onCancelClick }: { order: Order; onUpdateStatus: (id: string, newStatus: Order['status']) => void; onAssignDriverClick: (order: Order) => void; onCancelClick: (order: Order) => void; }) => {
    ...
    return (
        <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-lg shadow-xs border border-slate-200 p-3 hover:shadow-sm transition-shadow">
            ...
            <div className={`mt-2 flex items-center gap-2 text-xs font-semibold ${order.isDelivery ? 'text-blue-600' : 'text-purple-600'}`}>
                {order.isDelivery ? <Truck className="w-3.5 h-3.5"/> : <Store className="w-3.5 h-3.5"/>}
            ...
            {order.isDelivery && (
                <div className="mt-2 p-2 bg-blue-50 rounded text-blue-700 text-xs flex items-start gap-2">
                    <MapPin className="w-3.5 h-3.5 mt-0.5 flex-shrink-0"/> {order.deliveryAddress}
                </div>
            )}
            ...
            <div className="mt-3 space-y-2">
                {mainAction && (
                    <button onClick={mainAction.onClick} className={`w-full py-1.5 px-3 text-xs font-semibold text-white rounded-md shadow-sm transition ${mainAction.color}`}>
                        {mainAction.text}
                    </button>
                )}
...
const AssignDriverModal = ({ isOpen, onClose, drivers, onAssign }: { isOpen: boolean; onClose: () => void; drivers: Driver[]; onAssign: (driver: Driver) => void; }) => ( <Modal isOpen={isOpen} onClose={onClose} title="Atribuir Entregador"> {drivers.length === 0 ? ( <p className="text-slate-500">Você não tem entregadores cadastrados.</p> ) : ( <ul className="space-y-2"> {drivers.map(driver => ( <li key={driver.id}> <button onClick={() => onAssign(driver)} className="w-full text-left p-3 rounded-lg hover:bg-slate-100 transition flex items-center gap-3"> <div className="bg-blue-100 p-2 rounded-full"> <Truck className="w-4 h-4 text-blue-600" /> </div> <div> <p className="font-semibold text-slate-800">{driver.name}</p> <p className="text-xs text-slate-500">{driver.phone}</p> </div> </button> </li> ))} </ul> )} </Modal> );

const CancelOrderModal = ({ isOpen, onClose, onConfirm, order }: { isOpen: boolean; onClose: () => void; onConfirm: () => void; order: Order | null; }) => (
            ...
            <div className="mt-3 text-center sm:mt-5">
                ...
                <div className="mt-2">
                    <p className="text-sm text-gray-500">
                        Você tem certeza que deseja cancelar o pedido #{order?.id.substring(0, 6)}? Esta ação não pode ser desfeita.
                    </p>
                </div>
            </div>
        </div>

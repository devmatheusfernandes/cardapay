    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
          <h1 className="text-3xl font-bold text-slate-800">Gerenciamento do Cardápio</h1>
          <motion.button
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
            ...
            <select
                ...
                className="w-full md:w-64 border border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition py-2 px-3"
            >
                {categories.map(cat => (
...
const EmptyState = ({ onAddItem }: { onAddItem: () => void }) => (
    <div className="text-center py-20 px-6">
        ...
        <h3 className="mt-4 text-2xl font-semibold text-slate-800">Seu cardápio está vazio</h3>
        <p className="mt-2 text-slate-500">Comece adicionando seu primeiro prato ou bebida.</p>
        <div className="mt-6">
            <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={onAddItem}
                className="flex items-center gap-2 mx-auto px-5 py-2.5 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 transition"
            >
                <Plus className="w-5 h-5" />
...
const MenuItemCard = ({ item, onEdit, onDelete, onToggleStock }: { item: MenuItem, onEdit: (item: MenuItem) => void, onDelete: (item: MenuItem) => void, onToggleStock: (id: string, status: boolean) => void }) => (
  ...
  >
    <img src={item.imageUrl || 'https://placehold.co/600x400/EAEAEA/1A1A1A?text=Sem+Imagem'} alt={item.name} className="w-full h-48 object-cover" />
